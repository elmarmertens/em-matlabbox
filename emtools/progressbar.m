function progressbar(fractiondone, message, position)
% Description:
%   progressbar(fractiondone,message,position) provides an indication of the progress of
...
% with modifications by Elmar Mertens (message option, progressbar4xterm)
...
if ~usejava('desktop') || strcmp(get(groot, 'defaultFigureWindowStyle'), 'docked')
...
if ~isempty(message)
   titlebarstr = [titlebarstr, ' -- ', message];
end
